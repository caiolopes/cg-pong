...
class ShaderProgram {
    ...
    void setUniform(String uniformName, Matrix4f value) {
        ...
        if (uniformData == null) {
            throw new RuntimeException("Uniform [" + uniformName + "] nao foi criado");
        }
        // Checa se o float buffer foi criado
        FloatBuffer fb = uniformData.getFloatBuffer();
        if (fb == null) {
            fb = BufferUtils.createFloatBuffer(16);
            uniformData.setFloatBuffer(fb);
        }
        // Joga a matriz dentro do buffer de float
        value.get(fb);
    ...
    void setUniform(String uniformName, PointLight pointLight, int pos) {
    ...
    void setUniform(String uniformName, SpotLight spotLight, int pos) {
